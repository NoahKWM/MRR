...
We will try 3 approaches to deal with the strong correlations between the co-variables:
- No co-variables deletion (using Ridge regression to simplify the model)
- Backward Selection
...
We still use the same dataset, and we will use the Ridge method. We will use the best lambda which minimize
the MRSE.
...
We will use a k-fold method to build a model training with the 4 approaches, and predict the diagnosis. We will
use K = 5, meaning that 20% of the 569 observations will compose the test set and 80% will compose the training
set.
