...
for (k in (1:K)) {
  ...
  tabTrain <- tab[-indk, ];
  tmp_y <- tabTrain$diagnosis;
  tmp_X <- tabTrain[-1]; 
  modTrain <- glmnet(tmp_X, tmp_y, family = "binomial", alpha = 0, lambda = best_lambda_ridge);
  testk <- predict(modTrain, newx = tab[indk, ][-1], type = "response", s= best_lambda_ridge);
...
for (k in (1:K)) {
  ...
  testk_clear <- predict.glm(modTrain_clear, newdata = tab_clear[indk, ], type = "response");
